...
def test_fn_epoch(model, criterion, test_loader):
    ...
    savef = os.path.join(save_dir,'adjustlabel_predicts.csv')
    savea = os.path.join(save_dir,'origlabel_predicts.csv')
...
def log_summary(Pr2):
    ...
    savePr = os.path.join(save_dir,'Pr.csv')
...
def perform_predictions(config):
    global timestr
    ...
    predictOverlaps = config['PatchInfo']['PREDICT_OVERLAPS']
    ...
    if predictOverlaps == True:
        testSource = config['directories']['byMousePatches_withOverlaps']
        PREDICTIONS_DIR = os.path.join(BASE_DIR, 'predictions_wOverlaps')
        if not os.path.exists(PREDICTIONS_DIR):
            os.mkdir(PREDICTIONS_DIR)
    elif predictOverlaps == False:
        testSource = config['directories']['byMousePatches']
        PREDICTIONS_DIR = os.path.join(BASE_DIR,'predictions_noOverlaps')
        if not os.path.exists(PREDICTIONS_DIR):
            os.mkdir(PREDICTIONS_DIR)
    
    config['directories']['PREDICTIONS_DIR'] = PREDICTIONS_DIR
    ...
    for sample in samples:
        ...
        save_dir = os.path.join(PREDICTIONS_DIR,str(sample))
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
    
        img_test = [f for f in glob.glob(os.path.join(os.path.join(testSource,sample), '*png'))]
        #print(img_test)
