...
def generateProps(config):
    test_df = pd.read_csv(os.path.join(config['directories']['GEN_PATCHES_DIR'],'invUninvolvedPatchCounts_test.csv'))

    # sum up all patch counts in each
    test_df['totalCount'] = test_df.apply(lambda x: return_sum(a=[x['UNinvCount'],x['Cluster_0'],x['Cluster_1'],x['Cluster_2'],x['Cluster_3']]),axis=1)
