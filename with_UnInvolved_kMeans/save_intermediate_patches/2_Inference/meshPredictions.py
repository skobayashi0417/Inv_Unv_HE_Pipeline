...
def perform_Meshpredictions(config):
    global timestr
    ...
    testSource = config['directories']['byMouseMeshPatches']
    
    MESH_PREDICTIONS_DIR = os.path.join(BASE_DIR,'32pixelPatch_Predictions')
    if not os.path.exists(MESH_PREDICTIONS_DIR):
        os.mkdir(MESH_PREDICTIONS_DIR)
        
    config['directories']['meshPREDICTIONS_DIR'] = MESH_PREDICTIONS_DIR
    ...
    checkpoint_path = './2_Inference/smallPatch_model/smallPatch_chkpt.t7'
